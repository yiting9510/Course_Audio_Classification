...
class AudioDataset(Dataset):
    ...
    def __init__(self, label_files, class_dict, config, undersample=False):
    ...
    def _undersample(self):
        ...
        # Find the second most numerous class count
        sorted_counts, sorted_indices = torch.sort(label_counts, descending=True)
        second_max_count = sorted_counts[1].item()

        # Undersample to match the second most numerous class count
        new_pairs = []
        for label in range(len(label_counts)):
            ...
            if len(label_pairs) > second_max_count:
                label_pairs = np.random.choice(label_pairs, second_max_count, replace=False).tolist()
            new_pairs.extend(label_pairs)
...
class TrainAudioDataset(AudioDataset):
    # 重新定義建構式
    def __init__(self, label_files, class_dict, config):
        # 調用父類別的建構式
        super().__init__(
            label_files, class_dict, config=config, undersample=True
        )
